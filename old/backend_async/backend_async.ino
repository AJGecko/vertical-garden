...
LedEffect parseEffect(const String& effectName) {
    ...
    normalized.trim();
    if (normalized == "0" || normalized == "static" || normalized == "solid") return LED_EFFECT_STATIC;
    if (normalized == "1" || normalized == "blink" || normalized == "flash") return LED_EFFECT_BLINK;
    if (normalized == "2" || normalized == "breathe" || normalized == "breath") return LED_EFFECT_BREATHE;
    if (normalized == "3" || normalized == "rainbow") return LED_EFFECT_RAINBOW;
    if (normalized == "4" || normalized == "waves" || normalized == "wave") return LED_EFFECT_WAVES;
...
LedEffect parseEffectByIndex(int mode, LedEffect fallback) {
  switch (mode) {
    case 0:
      return LED_EFFECT_STATIC;
    case 1:
    ...
    case 4:
      return LED_EFFECT_WAVES;
    default:
      return fallback;
  }
}

LedEffect parseEffectFlexibleValue(JsonVariantConst value, LedEffect fallback) {
  if (value.isNull()) return fallback;
  if (value.is<int>() || value.is<long>() || value.is<unsigned int>() || value.is<unsigned long>() || value.is<float>()) {
    int mode = parseIntFlexible(value, (int)fallback);
    return parseEffectByIndex(mode, fallback);
  }
  String effectName = parseStringFlexible(value, effectToString(fallback));
  return parseEffect(effectName);
}

uint32_t parseEffectSpeedFlexible(JsonVariantConst effectSpeedMsValue,
                                  JsonVariantConst ledEffectSpeedMsValue,
                                  JsonVariantConst speedMsValue,
                                  uint32_t fallback) {
  int speedValue = parseIntFlexible(effectSpeedMsValue,
                   parseIntFlexible(ledEffectSpeedMsValue,
                   parseIntFlexible(speedMsValue, (int)fallback)));
  return clampU32((uint32_t)speedValue, 120, 10000);
}
...
void clearStripIfNeeded() {
  if (ledOutputInitialized && !lastRenderedOn) return;
  fillStrip(0, 0, 0);
  showStripSafely();
...
void applySolidIfChanged(LedEffect effect, uint8_t r, uint8_t g, uint8_t b) {
  if (ledOutputInitialized && lastRenderedOn && lastRenderedEffect == effect &&
      lastRenderedR == r && lastRenderedG == g && lastRenderedB == b) {
    return;
  }
  fillStrip(r, g, b);
  showStripSafely();
...
void handleApiLedPost(AsyncWebServerRequest *request) {
  ...
  if (!bodyStr || !parseJsonBodyAsync(*bodyStr, body)) {
    ...
    bool requestedOn = parseBoolFlexible(body["on"], parseBoolFlexible(body["ledStripOn"], ledState.userOn));
    uint8_t requestedR = (uint8_t)clampI32(parseIntFlexible(body["r"], parseIntFlexible(body["ledStripR"], ledState.r)), 0, 255);
    uint8_t requestedG = (uint8_t)clampI32(parseIntFlexible(body["g"], parseIntFlexible(body["ledStripG"], ledState.g)), 0, 255);
    uint8_t requestedB = (uint8_t)clampI32(parseIntFlexible(body["b"], parseIntFlexible(body["ledStripB"], ledState.b)), 0, 255);
    uint8_t brightnessPercent = (uint8_t)clampI32(parseIntFlexible(body["brightnessPercent"], parseIntFlexible(body["ledBrightnessPercent"], ledState.brightnessPercent)), 0, 100);
    ...
    if (body.containsKey("effect") || body.containsKey("ledEffect") || body.containsKey("ledEffectMode") || body.containsKey("mode")) {
      LedEffect nextEffect = ledState.effect;
      nextEffect = parseEffectFlexibleValue(body["mode"], nextEffect);
      nextEffect = parseEffectFlexibleValue(body["ledEffectMode"], nextEffect);
      nextEffect = parseEffectFlexibleValue(body["ledEffect"], nextEffect);
      nextEffect = parseEffectFlexibleValue(body["effect"], nextEffect);
      ledState.effect = nextEffect;
    }
    if (body.containsKey("effectSpeedMs") || body.containsKey("ledEffectSpeedMs") || body.containsKey("speedMs")) {
      ledState.effectSpeedMs = parseEffectSpeedFlexible(body["effectSpeedMs"], body["ledEffectSpeedMs"], body["speedMs"], ledState.effectSpeedMs);
    }
    ...
    response["ledStripOn"] = ledState.userOn;
    response["ledStripR"] = ledState.r;
    response["ledStripG"] = ledState.g;
    response["ledStripB"] = ledState.b;
    response["ledBrightnessPercent"] = ledState.brightnessPercent;
...
void handleGardenSettingsPost(AsyncWebServerRequest *request) {
  ...
  if (!bodyStr || !parseJsonBodyAsync(*bodyStr, body)) {
    ...
    if (body.containsKey("ledEffect") || body.containsKey("effect") || body.containsKey("ledEffectMode") || body.containsKey("mode")) {
      LedEffect nextEffect = ledState.effect;
      nextEffect = parseEffectFlexibleValue(body["mode"], nextEffect);
      nextEffect = parseEffectFlexibleValue(body["ledEffectMode"], nextEffect);
      nextEffect = parseEffectFlexibleValue(body["effect"], nextEffect);
    ...
    if (body.containsKey("ledEffectSpeedMs") || body.containsKey("effectSpeedMs") || body.containsKey("speedMs")) {
      ledState.effectSpeedMs = parseEffectSpeedFlexible(body["effectSpeedMs"], body["ledEffectSpeedMs"], body["speedMs"], ledState.effectSpeedMs);
    }
